#ifndef TR7_TR7NUMERIC_H
#define TR7_TR7NUMERIC_H
#pragma once
...
//
// Contains useful numerical algorithms.
//
...
namespace tr7
{
    ...
    enum class numeric_exitflag : int
    {
        success = 1,
        converged_in_x = 2,

        found_nan_or_inf = -3,
        possibly_converged_to_singular_point = -5,
        no_sign_change_found = -6,
        max_function_evaluations_reached = -100,
        same_sign_at_domain_limits = -200

    };


    //
    // Outr "numeric" functions.
    //

    template<typename Fun,
             typename T,
             typename TolType = T>
    inline std::tuple<T, T, numeric_exitflag> fzero(Fun fun, T x0,
...
#define FZERO_CHECK_FUN_EVALS { ++fun_evals; if (fun_evals > max_fun_evals) { exitflag = numeric_exitflag::max_function_evaluations_reached;  goto ret; } }
        ...
        auto exitflag = numeric_exitflag::success;

        auto x = x0;
        auto fval = fun(x); FZERO_CHECK_FUN_EVALS;

        if (!std::isfinite(fval)) {
            exitflag = numeric_exitflag::found_nan_or_inf;
            ...
            exitflag = numeric_exitflag::no_sign_change_found;
            x = std::numeric_limits<T>::quiet_NaN();
            fval = std::numeric_limits<T>::quiet_NaN();
            goto ret;
        ...
        if (eq_fp(fval, T{ 0 }, tolf_e)) {
            ...
            if (guard) {
                ...
                if (std::abs(fval) > b_q) {
                    exitflag = numeric_exitflag::possibly_converged_to_singular_point;
                }
    ...
    ret:
        return std::make_tuple(x, fval, exitflag);
...
#undef FZERO_CHECK_FUN_EVALS
    ...
    template<typename Fun,
             typename T,
             typename TolType = T>
    inline std::tuple<T, T, numeric_exitflag> fzero(Fun fun, T a, T b,
                                                    std::size_t max_fun_evals = 10000u,
                                                    TolType tolx = TolType{ 1e-12 },
                                                    TolType tolf = TolType{ 1e-12 })
    {
...
#define FZERO_CHECK_FUN_EVALS { ++fun_evals; if (fun_evals > max_fun_evals) { exitflag = numeric_exitflag::max_function_evaluations_reached;  goto ret; } }
        ...
        if (samesign(fval, fa)) {
            exitflag = numeric_exitflag::same_sign_at_domain_limits;
        }
...
#undef FZERO_CHECK_FUN_EVALS
    ...
    inline auto zeroin(Args&&... args)
    {
        //
        // A common alias for "fzero" within an interval.
        //

        return fzero(std::forward<Args>(args)...);
    ...
    constexpr std::tuple<T, T, numeric_exitflag> Newton_method(Fun fun, dFun_dx dfun_dx, T x0,
                                                               T alpha = T{ 1 },
                                                               std::size_t max_fun_evals = 10000u,
                                                               TolType tolx = TolType{ 1e-12 },
                                                               TolType tolf = TolType{ 1e-12 })
    {
        ...
        auto fval = fun(x0);
        if (eq_fp(fval, T{ 0 }, tolf_e)) {
            return std::make_tuple(x0, fval, exitflag);
        ...
        for (std::size_t fun_evals = 2u; fun_evals <= max_fun_evals; ++fun_evals) {
            ...
            if (eq_fp(fval, T{ 0 }, tolf_e)) {
                ...
                exitflag = numeric_exitflag::converged_in_x;
                return std::make_tuple(x0, fval, exitflag);
        ...
        exitflag = numeric_exitflag::max_function_evaluations_reached;
        return std::make_tuple(x0, fval, exitflag);
    ...
    template<typename Fun,
             typename T, typename TolType = T>
    constexpr std::tuple<T, numeric_exitflag> fixed_point_iteration(Fun fun, T x0,
                                                                    std::size_t max_fun_evals = 10000u,
                                                                    TolType tolx = TolType{ 1e-12 })
    {
        //
        ...
        if (!std::isfinite(x0)) {
            exitflag = numeric_exitflag::found_nan_or_inf;
            x0 = std::numeric_limits<T>::quiet_NaN();
            return std::make_tuple(x0, exitflag);
    ...
    constexpr std::tuple<std::array<T, 2>,
                         std::array<T, 2>,
                         numeric_exitflag> numjac_Newton_method2d(Fun2d fun2d,
                                                                  std::array<T, 2> x0,
                         ...
                         std::size_t> cubic_roots(RealType a, RealType b, RealType c, RealType d)
    {
        ...
        return std::make_tuple(std::array<ComplexType, 3>{ Z,
                         ...
                         std::size_t> quartic_roots(RealType a, RealType b, RealType c, RealType d, RealType e)
    {
        ...
        if (Da >= RealType{ 0 } && Db >= RealType{ 0 }) {
            return std::make_tuple(std::array<ComplexType, 4>{ (-p_a + sqrtDa) * RealType{ 0.25 },
            ...
            return std::make_tuple(std::array<ComplexType, 4>{ (-p_b + sqrtDb) * RealType{ 0.25 },
                                                               (-p_b - sqrtDb) * RealType{ 0.25 },
                                                               (-p_a + sqrtDa) * RealType{ 0.25 },
                                                               (-p_a - sqrtDa) * RealType{ 0.25 } },
                                   std::size_t{ 2 });
